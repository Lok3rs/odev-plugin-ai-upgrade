...
CLI args (declared on `UpgradeCommand`): `module_name` (positional, optional), `--path`, `--to`, `--from`, `-c/--comment`, `--task-id` (**required**), `--submodules`, `--no-ruff`, `--ultracode`, `--effort`. `cli`/`model`/`yolo`/`headless`/`resume`/`dirs` come from `AICommandMixin`.
